...
class Stack_branch_and_bound(list): 
    """Estructura de datos para resolver el algorítmo branch and bound"""
    
    def __init__(self,*args):
        super().__init__(*args)

    def extend(self, items):
        super().extend(items) #Se añaden los nodos a la lista#
        self.sort(reverse=True) #Se ordenan de menor a mayor# # Vease la clase Node para ver como es el funcionamiento de la comparaciones entre nodos


class stack_branch_and_bound_heuristics(Stack_branch_and_bound): 
    """Estructura de datos para resolver el algorítmo branch and bound subestimado"""
    def __init__(self, problem):
        super().__init__()
        self.problem=problem  #Atributo problem que utilizamos para calcular la distancia euclidea#
    
    def extend(self, items) -> None:
        expanded_list = [] #Lista de nodos expandidos#

        for node in items:    
            tupla = (node.__dict__["path_cost"] + self.problem.h(node),node) #Tuplas con la primera posición la suma de coste acumulado más el valor heuristico(distancia euclidea) y en la segundo posición el nodo#
            expanded_list.append(tupla) #Se añade cada tupla#
        
        return super().extend(expanded_list) #Se añaden las tuplas a la estructura base#
    
    def pop(self):
        """Hacemos un pop de la tupla y devolvemos el nodo segundo posición[1]"""
        return super().pop()[1] 
    
    def append(self, node):
        """Método append básico"""
        return super().append((node.__dict__["path_cost"] + self.problem.h(node),node)) 
